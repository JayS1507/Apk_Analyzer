...
class APKAnalyzer:
    ...
    async def analyze_apk(self, apk_path: str, output_dir: str, password: Optional[str] = None) -> Dict[str, Any]:
        ...
        # Use available tools for analysis
        if self.tools_available['apktool']:
            result.update(await self._analyze_with_apktool(apk_path, output_dir))
            result["analysis_tools_used"].append("apktool")
        
        if self.tools_available['jadx']:
            await self._analyze_with_jadx(apk_path, output_dir)
            result["analysis_tools_used"].append("jadx")
        
        if self.tools_available['androguard']:
            result.update(await self._analyze_with_androguard(apk_path))
            result["analysis_tools_used"].append("androguard")
        
        # Extract additional patterns from decompiled code
        if self.tools_available['jadx']:
            result.update(await self._extract_patterns_from_code(output_dir))
        
        # Malware-focused enrichment
        result["malware_profile"] = await self._build_malware_profile(output_dir, result)

        # External intelligence enrichers (optional via env)
    ...
    async def _analyze_with_jadx(self, apk_path: str, output_dir: str):
        ...
        try:
            ...
            if jadx_path.exists():
                # Use better JADX options for comprehensive decompilation
                base = [str(jadx_path), '-d', jadx_output, '--show-bad-code', '--deobf', '--deobf-rewrite-cfg', apk_path]
                cmd = ['cmd', '/c', *base] if is_windows else base
            else:
                base = ['jadx', '-d', jadx_output, '--show-bad-code', '--deobf', '--deobf-rewrite-cfg', apk_path]
                cmd = ['cmd', '/c', *base] if is_windows else base
    ...
    async def _build_malware_profile(self, output_dir: str, base: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for u in urls:
            lu = u.lower()
            if any(x in lu for x in ["pool.", "nicehash", ":9000", "mining"]):
        ...
        # Wallet detection (Monero wallets typically 95 chars, often starting with '4')
        monero_wallet_regex = r"\b[48][A-Za-z0-9]{93}\b"
        wallet_hits: List[str] = []

        # Scan code for additional indicators
        ...
        for root in search_roots:
            ...
            for r, d, files in os.walk(root):
                for f in files:
                    ...
                    # Wallets
                    import re as _re
                    for m in _re.findall(monero_wallet_regex, content):
                        if m not in wallet_hits:
                            wallet_hits.append(m)
        ...
        if any("usesCleartextTraffic\"\=" in u for u in lower_urls) or any("http://" in u for u in lower_urls):
            profile["obfuscation_evasion"].append("Cleartext traffic observed")

        # Interesting strings / flags
        ...
        for s in suspicious_strings:
            if any(flag in s.lower() for flag in ["monero", "wallet", "nicehash", "miner"]):
                profile["interesting_strings"].append(s)
        for bucket in [profile["network"]["data_exfiltration"], profile["network"]["mining_pools"], urls]:
            for u in bucket:
                if any(f in u.lower() for f in ["monero", "wallet", "nicehash", ":9000", "miner"]):
                    profile["interesting_strings"].append(u)
        profile["interesting_strings"] = list(dict.fromkeys(profile["interesting_strings"]))
        ...
        for u in urls:
            try:
                ...
                if host:
                    ...
                    if parsed.port:
                        profile["iocs"]["ips_ports"].append(f"{host}:{parsed.port}")
            except Exception:
                continue
        profile["iocs"]["domains"] = sorted(list(set(domain_candidates)))[:100]
        profile["iocs"]["ips_ports"] = sorted(list(set(profile["iocs"]["ips_ports"])))[:100]
        profile["iocs"]["wallets"] = wallet_hits[:5]
        ...
        profile["fcmi_chain"]["dropped_binary_names"] = dropped_names[:5]
    ...
    async def _run_external_enrichers(self, apk_path: str, base: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Gather IOCs
        domains = base.get("malware_profile", {}).get("iocs", {}).get("domains", [])
        ips_ports = base.get("malware_profile", {}).get("iocs", {}).get("ips_ports", [])
        wallets = base.get("malware_profile", {}).get("iocs", {}).get("wallets", [])

        tasks = []
        async with httpx.AsyncClient(timeout=20.0) as client:
            # VirusTotal
            vt_key = _os.getenv("VT_API_KEY")
            if vt_key:
                tasks.append(self._vt_lookup(client, vt_key, sha256, domains, ips_ports))
        ...
        for r in results or []:
            try:
                if isinstance(r, dict) and r.get("_kind") == "vt":
                    ...
                    intel["securitytrails"] = r.get("data")
